...
public class ConfigureMultiSlide {
    ...
    public ConfigureMultiSlide(String install_dir, String java_dir, String os) {
        ...
        if (this.os_type == ConfigureMultiSlide.WINDOWS_OS) {
    ...
    public void run(){
        ...
        } else if (this.os_type == ConfigureMultiSlide.UNIX_OS ||
                    this.os_type == ConfigureMultiSlide.MAC_OS) {
            glassfish_dir = findFile(
                                   "Please Provide Path to Glassfish Server Installation Directory:",
                                   "asadmin",
                                   "Glassfish Server Installation Directory"
        ...
        println("Generating multiSLIDE configuration files...");
        boolean configCreated = createSlideConfig(java_dir, glassfish_dir, mongo_dir, python_dir);
        if (!configCreated) {
            println("multiSLIDE configuration failed.");
            System.exit(-1);
        } else {
            println("Done.");
        }
        ...
        try {
            println("Generating multiSLIDE web configuration files...");
            addConfigToJar();
            println("Done.");
        } catch (Exception e) {
        ...
        println("multiSLIDE configured successfully!");
        if (this.os_type == ConfigureMultiSlide.WINDOWS_OS) {
            println("To start multiSLIDE run " + install_dir + File.separator + "bin" + File.separator + "start-slide.bat");
            println("To stop multiSLIDE run " + install_dir + File.separator + "bin" + File.separator + "stop-slide.bat");
        } else if (this.os_type == ConfigureMultiSlide.UNIX_OS ||
                    this.os_type == ConfigureMultiSlide.MAC_OS) {
            println("To start multiSLIDE run " + install_dir + File.separator + "bin" + File.separator + "start-slide.sh");
            println("To stop multiSLIDE run " + install_dir + File.separator + "bin" + File.separator + "stop-slide.sh");
        }
    }
    ...
    public boolean checkMongoStorage(String mongo_dir) {
        ...
        switch (this.os_type) {
            case ConfigureMultiSlide.WINDOWS_OS:
                mongo_storage_path = mongo_dir.substring(0, 1) +
                        ":" + File.separator + "data" + File.separator + "db";
                break;
            case ConfigureMultiSlide.MAC_OS:
            case ConfigureMultiSlide.UNIX_OS:
    ...
    public boolean createSlideConfig (String java_dir, String glassfish_dir, String mongo_dir, String python_dir) {
        ...
        try {
          ...
          println(e);
          println("Unable to create multiSLIDE configuration file.");
          return false;
        }
    ...
    public boolean importDataIntoMongoDB(String mongo_dir) {
        ...
        try {
            ...
            String[] collection_names = {"pathwayMap", "goMap2", "geneMap2", "entrezAliasMap", "aliasEntrezMap", "ensemblEntrezMap", "ensemblprotEntrezMap", "ensembltranscriptEntrezMap", "EntrezMap", "geneMap", "goMap", "refseqEntrezMap", "uniprotEntrezMap", "PPI", "TFTargetMap", "miRGOMap", "miRNATomirFamily", "miRPathwayMap", "mirFamilyToTargets"};
            ...
            for (int i=0; i<collection_names.length; i++) {
                ...
                print("\tImporting collection " + (i+1) + " of 19...");
