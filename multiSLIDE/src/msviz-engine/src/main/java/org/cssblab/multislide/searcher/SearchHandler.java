...
public final class SearchHandler {
    ...
    public static HashMap <String, ArrayList <SearchResultObject>> processSearchQuery (
            String queryStr, Searcher searcher
        ...
        if(queryStr.contains(";")){
            ...
            for(int i = 0; i < outer_list.length; i++){
                ...
                ArrayList <SearchResultObject> current_search_results = processQuery(query, searcher);
    ...
    public static ArrayList <SearchResultObject> processQuery(
            String query, Searcher searcher
    ) throws MultiSlideException {
        ...
        if(query.contains(":")) { // wildcard
            ...
            return doSearch(keyword, search_type, search_term, searcher);
    ...
    public static ArrayList <SearchResultObject> doSearch(
            String query_type, String search_type, String search_term, Searcher searcher
    ) throws MultiSlideException  {
        ...
        if (query_type != null) {
            if (query_type.equals("pathid") || query_type.equals("pathname")) {
                while (st.hasMoreTokens()) {
                    ...
                    // add an IF term to check if there is miRNA data
