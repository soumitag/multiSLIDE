...
public class Lists {
    ...
    public String serializeFeatureList(String name, String identifier, String delimval) throws MultiSlideException {
        if (hasFeatureList(name)) {
            String str = "";
            ArrayList <String[]> list = featureListMap.get(name);
            for (int i=0; i<list.size(); i++) {
    ...
    public String getListsMetadata() {
        //ListData list_data = new ListData();
        return ListData.getAsJSON(this.featureListMap, "feature_list");
    }
}
