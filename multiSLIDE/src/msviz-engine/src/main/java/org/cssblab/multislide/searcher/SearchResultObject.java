...
public class SearchResultObject implements Serializable {
    ...
    public int compareTo(SearchResultObject that) {
        int c = this.num_common_genes - that.num_common_genes;
        if (c != 0) {
            return c;
        } else {
    ...
    public static ArrayList <SearchResultObject> sortSearchResultObjects(ArrayList <SearchResultObject> search_results, HashMap <String, Boolean> entrezMaster) {
        for (int i=0; i<search_results.size(); i++) {
            search_results.get(i).computeNumGeneIntersections(entrezMaster);
    ...
    public void computeNumGeneIntersections(HashMap <String, Boolean> entrezMaster) {
    ...
    public SearchResultSummary getSummary () {
        ...
        if (this.type == SearchResultObject.TYPE_GENE) {
            GeneObject gene = (GeneObject) this.result;
            summary.createSearchResultSummary(SearchResultSummary.TYPE_GENE_SUMMARY, gene.gene_identifier, gene.entrez_id, 1, this.num_common_genes);
            summary.createHTMLDisplayTag(query);
        } else if (this.type == SearchResultObject.TYPE_GO) {
            GoObject ontology = (GoObject) this.result;
            summary.createSearchResultSummary(
                    SearchResultSummary.TYPE_GO_SUMMARY, ontology.goTerm, ontology.goID, ontology.nGenes, this.num_common_genes
            );
            summary.createHTMLDisplayTag(query);
        } else if (this.type == SearchResultObject.TYPE_PATHWAY) {
            PathwayObject pathway = (PathwayObject) this.result;
            summary.createSearchResultSummary(
                    SearchResultSummary.TYPE_PATH_SUMMARY, pathway.pathway, pathway._id, pathway.nGenes, this.num_common_genes
            );
            summary.createHTMLDisplayTag(query);
        }
